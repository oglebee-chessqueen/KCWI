...
####### Grating constants ########
nref = 1.35			# index of refraction of the gelatin
...
beta = numpy.zeros( len(alpha) )		# Determine output angle
wv0 = numpy.zeros( len(alpha) )
for i in range(0,len(alpha)):
	theta = math.asin( math.sin(alpha[i]*d2r)/nref )
	eta = efficiency(theta, nref, bta, dn, thick, waves)
	w0 = waves[eta==numpy.max(eta)]
	wv0[i] = w0[0]
	#print wv0
	#~ beta[i] = math.asin( nref * math.sin( (math.asin(wv0*r/nref) - math.sin(theta + delta) ) ) ) / d2r
	beta[i] = math.asin( (wv0[i]*r - math.sin(alpha[i]*d2r)) ) / d2r
	print "wave: %0.1f , alpha = %0.1f \t beta = %0.1f" % (wv0[i]*1000,alpha[i], beta[i])
...
for i in range(0,numpy.size(waves)):
	...
	if (nref * math.sin( math.asin(waves[i] * r/2./nref) + delta )) >= 1.0:
		...
		braggs_eff[i] = efficiency(theta, nref, bta[i], dn, thick, waves[i])
		#~ print waves[i], braggs/d2r, braggs_eff[i]
		# Also determine resolution here
		resol[i] = (F*D*waves[i]*r)/math.cos(braggs)
...
if len(i_70) == 0:
	i_70 = numpy.where(braggs_eff > numpy.max(braggs_eff)*0.7)[0]
i_70_min = i_70[0]
i_70_max = i_70[-1]
...
if wv_mx > 1.08:
	wv_mx = 1.08
print "Peak Eff:"
print "wave = %0.2f \t Braggs = %0.2f \t eff = %0.6f" % (waves[i_mx]*1000,  brg_mx, braggs_eff[i_mx])
print "70% Eff:"
print "wave = %0.2f \t eff = %0.6f" % (wv_mn*1000, braggs_eff[i_70_min])
print "wave = %0.2f \t eff = %0.6f" % (wv_mx*1000, braggs_eff[i_70_max])
...
for k in range(0,len(wv0)):
	plt.text(wv0[k]*1000 - 10, braggs_eff[numpy.where(waves==wv0[k])[0]]*1.055,
					 r'$\alpha$ = %0.2f$^{\circ}$'%(alpha[k]), color='grey' )
	plt.text(wv0[k]*1000 - 10, braggs_eff[numpy.where(waves==wv0[k])[0]]*1.02,
					 r'$\beta$ = %0.2f$^{\circ}$'%(beta[k]), color='grey' )
...
plt.plot(waves[i_70_min:i_70_max]*1000.,resol[i_70_min:i_70_max],color='black')
plt.xlim(500.,1100.)
plt.title(gtg,fontsize=20)
plt.xlabel('Wavelength (nm)',fontsize=14)
