...
#~ alpha = numpy. array( [18.0, 16.0, 13.79] )# RL AOI
...
## Medium: RM1 + RM2
#~ gtg = 'KCRM RM v1'
#~ gtg_ind = ['KCRM RM1 v1','KCRM RM2 v1']
#~ res_avg = 1900
...
gtg = 'KCRM RH v1'
gtg_ind = ['KCRM RH1 v1','KCRM RH2 v1', 'KCRM RH3 v1','KCRM RH4 v1']
res_avg = 4600
...
# A: Per angle setting (set alpha; use tilt to determine beta and efficiency)
	...
	for k in range(0,len(alpha)):
		if abs(alpha[k] - beta[g,k]) < 6.5:
			...
			clr = 'darkgreen'#'grey'
		plt.text(wv0[g,k]*1000 - 10, braggs_eff[g,numpy.where(waves==wv0[g,k])[0]]*1.05,
					 r'$\alpha$ = %0.2f$^{\circ}$'%(alpha[k]), color=clr, fontsize = 8 )
		plt.text(wv0[g,k]*1000 - 10, braggs_eff[g,numpy.where(waves==wv0[g,k])[0]]*1.025,
					 r'$\beta$ = %0.2f$^{\circ}$'%(beta[g,k]), color=clr, fontsize = 8 )
	...
	if len(r_suite) > 0:	#== 1:
		...
		plt.text(550,0.35, r'$\lambda_c$ = %i nm'%(waves[i_mx]*1000),
				 bbox=dict(facecolor='white', edgecolor='white', alpha=0.9),
				 fontsize = 10)
		plt.text(750,0.35, r'$\lambda_{min}$ = %i nm'%(wv_mn*1000),
				 bbox=dict(facecolor='white', edgecolor='white', alpha=0.9),
				 fontsize = 10)
		plt.text(550,0.275, r'd = %0.2f $\mu$m'%(thick),
				 bbox=dict(facecolor='white', edgecolor='white', alpha=0.9),
				 fontsize = 10)
		plt.text(750,0.275, r'$\lambda_{max}$ = %i nm'%(wv_mx*1000),
				 bbox=dict(facecolor='white', edgecolor='white', alpha=0.9),
				 fontsize = 10)
