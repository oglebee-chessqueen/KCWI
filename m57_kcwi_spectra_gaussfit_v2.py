...
for i in range(0,numpy.size(data4[0,:,0])-1,dec_bin):
	...
	for j in range(0,numpy.size(data4[0,0,:])-1,ra_bin):
		...
		for ww in linewave:
			...
			#print ww, i_linewave
			amp = abs(numpy.max(spectra_0[i_linewave[0]-4:i_linewave[0]+4]))
			if numpy.isnan(amp):
		...
		print guess.reshape((len(guess)/3,3))
		print
		popt, pcov = curve_fit(gaussum, waves4, spectra_0, p0=guess)#, sigma = err)
															   # this optimizes the summed gaussian function
		fit = gaussum(waves4, *popt)
		#guess = popt		# for all other iterations after first set
		...
		plt.plot(waves4, spectra_0, drawstyle='steps-mid', lw=3,
				 color='black')
		#~ plt.plot(waves4, continuum_flux, drawstyle='steps-mid', lw=3, color='blue')
		#~ plt.plot(wave_bin, flux_bin, 'ro', ms = 5)
		plt.plot(waves4, fit, color='green', drawstyle='steps-mid', lw=5)
		# also plot individual Gaussian fits
		# Use this loop to determine v_r, fwhm from Gaussian fits
		v_r = numpy.zeros(len(popt[:,0]))
		fwhm = numpy.zeros(len(popt[:,0]))
		for i in range(0, len(popt[:,0])):
			if numpy.any(lineID.index('Buffer')) == i:
				continue
			else:
				v_r[i] = ((popt[i,1] - linewave[i]) / linewave[i])*c
				fwhm[i] = abs(2.3548*popt[i,2])		# in wavelength (Ang) units
			gfit = gaus(waves4,popt[i,0],popt[i,1],popt[i,2])
